...
public class MemberServiceImpl implements MemberService {
    ...
    @Value("${spring.cloud.aws.s3.bucket:default-bucket}")
    ...
    public MemberServiceImpl(
            java.util.Optional<S3Service> s3Service, // 로컬에선 빈이 없으므로 Optional.empty()가 들어옴
        ...
        this.s3Service = s3Service.orElse(null); // 로컬에선 null이 됨
            ...
            if (role.getRoleType().equals(RoleType.ROLE_SELLER)) {
                ...
                for (Goods goods : goodsList) {
                    ...
                    for (Image image : imageList) {
                        ...
                        // 4. Null 체크 추가 (S3Service가 로컬에선 null이므로)
                        if (s3Service != null) {
