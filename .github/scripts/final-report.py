...
[작성 지침]
1. **Critical 취약점 중 가장 위험한 5개**만 상세히 작성하세요.
2. **High 취약점 중 가장 위험한 5개**만 상세히 작성하세요.
3. 우선순위 기준: 하드코딩된 비밀번호 > 인젝션 > 인증/권한 > 기타
4. 각 취약점별로 다음 형식을 따르세요:
   - **[위치]**: 파일명 및 라인 번호
   - **[취약점 설명]**: 어떤 보안 위험이 있는지 짧고 명확하게 설명
   - **[시큐어 코딩 가이드]**: '잘못된 코드' vs '수정된 코드' 예시를 마크다운 코드 블록으로 제시
5. 말투는 친절하고 전문적인 톤으로 유지하세요.
6. 마지막에 "📊 추가 발견: Critical N개, High N개, Medium N개가 더 있습니다" 요약을 추가하세요.
7. 전체적인 보안 품질 향상을 위한 조언을 한 줄 추가하세요.
8. 결과는 GitHub PR 댓글에 바로 붙일 수 있도록 마크다운(Markdown) 형식으로만 출력하세요.
